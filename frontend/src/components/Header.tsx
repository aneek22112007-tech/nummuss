  ...
  return (
    ...
    >
      ...
      <nav className="hidden items-center gap-6 lg:flex">
        ...
        <div className="flex items-center ml-2">
          ...
          {!isAuthenticated && (
            <button
              onClick={onSignIn}
              className="rounded-full bg-mint px-5 py-2 text-[13px] font-bold tracking-wide text-charcoal transition-transform hover:scale-105 active:scale-95"
            >
              Sign In
            </button>
          )}
              ...
              <AnimatePresence>
                {userMenuOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-48 overflow-hidden rounded-xl border border-snow/10 bg-[#111412] shadow-2xl backdrop-blur-xl"
                  >
      ...
      <button
        ...
        className="fixed right-4 top-4 z-50 grid h-12 w-12 place-items-center rounded-full bg-charcoal/80 text-snow border border-snow/10 backdrop-blur-md transition-all duration-300 hover:bg-snow/10 lg:hidden"
      >
